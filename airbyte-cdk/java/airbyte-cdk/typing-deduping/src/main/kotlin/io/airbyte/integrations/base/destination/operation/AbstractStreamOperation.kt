...
abstract class AbstractStreamOperation<DestinationState : MinimumDestinationState, Data>(
        ...
        if (isTruncateSync) {
            if (destinationInitialStatus.initialTempRawTableStatus.rawTableExists) {
                ...
                if (tempStageGeneration == null || tempStageGeneration == stream.generationId) {
                    log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: truncate sync, and existing temp raw table belongs to generation $tempStageGeneration (== current generation ${stream.generationId}). Retaining it." }
                    ...
                    log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: truncate sync, and existing temp raw table belongs to generation $tempStageGeneration (!= current generation ${stream.generationId}). Truncating it." }
                ...
                log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: truncate sync, and no preexisting temp raw table. Creating it." }
            ...
            log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: non-truncate sync. Creating raw table if not exists." }
            storageOperation.prepareStage(stream.id, NO_SUFFIX)
            if (destinationInitialStatus.initialTempRawTableStatus.rawTableExists) {
                log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: non-truncate sync, but temp raw table exists. Transferring it to real raw table." }
                ...
                log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: After record transfer, initial raw table status is $initialRawTableStatus." }
            } else {
                initialRawTableStatus = destinationInitialStatus.initialRawTableStatus
                log.info { "${stream.id.originalNamespace}.${stream.id.originalName}: non-truncate sync and no temp raw table. Initial raw table status is $initialRawTableStatus." }
            }
        }
    ...
    override fun finalizeTable(streamConfig: StreamConfig, syncSummary: StreamSyncSummary) {
        ...
        // Normal syncs should T+D regardless of status, so the user sees progress after every attempt.
        // We know this is a normal sync, so initialRawTableStatus is nonnull.
        if (!isTruncateSync && syncSummary.recordsWritten == 0L && !initialRawTableStatus!!.hasUnprocessedRecords) {
            log.info {
            ...
            log.info { "Skipping typing and deduping for stream ${streamConfig.id.originalNamespace}.${streamConfig.id.originalName}. Stream success: $streamSuccessful; records written: ${syncSummary.recordsWritten}" }
        } else {
